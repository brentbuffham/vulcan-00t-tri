#!/usr/bin/env python3
"""Beam v3: complete framing (generalized prefix: nb=(p&7)+1, depth=p>>3).
Branch only over axis (depth prescribed by token). GT scoring as beam2."""
import sys, struct
import numpy as np
...
for t in toks:
    ...
    if nb+depth>8:
        vi+=1
        beam=[(sc,rx,ry,rz,ph+((-1,0,False),)) for sc,rx,ry,rz,ph in beam]
        continue
    cand={}
    for sc,rx,ry,rz,ph in beam:
        ...
        for a in range(3):
            r=regs[a]
            vb=r[:8-nb-depth]+payload+(r[8-depth:] if depth else b'')
            v=be(vb)
            ok=gt_hit(v,a)
            ...
            if key not in cand or cand[key][0]<s2:
                cand[key]=(s2,rr[0],rr[1],rr[2],ph+((a,depth,ok),))
...
print(f'TOTAL: {allh}/{alln} = {allh/alln*100:.1f}%   (beam2 was 83.9%)')
np.save(r'C:\Users\brent\AppData\Local\Temp\claude\C--Users-brent-desktop-git-vulcan-00t-tri\6045a728-103e-40d9-818d-ffe48c95ac26\scratchpad\beam3_path.npy',
