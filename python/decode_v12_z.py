...
# §7.2 residual filter (GT-free): drop pass-(n-1) vertices whose Z disagrees
# with the ANCHOR-ONLY surface by > DROP_TH metres before re-decoding, so
# band-corrupted priors stop poisoning their neighbourhoods. DROP_TH=0 = off.
DROP_TH = float(sys.argv[5]) if len(sys.argv) > 5 else 0.0
for pn in range(2, NPASS + 1):
    dropped = 0
    if DROP_TH > 0:
        ag = Grid(); ag.set_cell(cell if cell else 8.0)   # anchor-only surface
        for vi in range(len(P)):
            if ANCH[vi] > 0:
                ag.add(P[vi, 0], P[vi, 1], P[vi, 2], vi, 1)
        keep = np.ones(len(P), bool)
        for vi in range(len(P)):
            if ANCH[vi] > 0:
                continue
            pr = plane_ref(ag, P[vi, 0], P[vi, 1], exclude=vi)
            # only drop when the anchor surface is CONFIDENT (small IQR, metres)
            if pr is not None and pr[1] <= 0.5 and abs(P[vi, 2] - pr[0]) > DROP_TH:
                keep[vi] = False; dropped += 1
    else:
        keep = np.ones(len(P), bool)
    ...
    print(f'pass{pn}: decoded {len(P)} vertices  {_passscore(P)}  (filter dropped {dropped})')
