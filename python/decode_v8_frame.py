#!/usr/bin/env python3
"""decode_v8_frame -- v7 cold pipeline + FRAMING FIX (GT-free, byte-role rules).

Misframe root cause (MISFRAME_CENSUS.md): the v7 Fe branch consumed a fixed 10
bytes (escape + 8-byte FULL + 1 trailer). Byte census on intercepts AND SYLVANIA
shows the trailer is an OPTIONAL SEP byte ((b&7)==7): every observed trailer is
0x_7/0x_F except where the next record starts immediately (0x40/41/C0/C1 = next
FULL's lead -- which v7 swallowed, desyncing the stream and corrupting registers;
also 0x20 17 / 0x38 ef = next TAG-SEP records).

v8 framing rules (tokenize):
  1. escape+FULL: ANY lead byte with an in-band FULL at pos+1 (v7 required
     b>=0x20; b<0x20 escapes went through V-truncation with inconsistent length).
     Consumes 9 bytes + 1 more IFF the following byte is SEP-class ((b&7)==7).
  2. (variant b only) V records always take their declared nb bytes -- no
     mid-payload FULL truncation.
Everything else identical to decode_v7_cold.py. GT (DXF) for SCORING ONLY.
Usage: python decode_v8_frame.py <case> [a|b]
"""
import struct, sys
import numpy as np
...
# ---- dynamic container location (unchanged) ----
...
print(f'{case} (v8{VARIANT}): seed_off {seed_off} coord_start {coord_start} face_start {face_start}  seeds {[round(s,1) for s in SEED]}')
...
# ---- v8 tokenizer: escape+FULL with optional SEP trailer ----
def tokenize(full_at):
    ...
    while pos < face_start:
        ...
        if full_at(pos + 1) is not None:  # escape+FULL, ANY escape byte
            toks.append(('Fe', d[pos + 1:pos + 9], lastT, None)); lastT = None
            pos += 9
            if pos < face_start and (d[pos] & 7) == 7: pos += 1  # optional SEP trailer
            continue
        if b < 0x20:
...
# ---- shorth bands from framed FULLs (unchanged) ----
...
for t in toks:
    ...
    ph = (ph + 1) % 3
P = np.array(pts)
np.save(f'P_v8{VARIANT}_{case}.npy', P)
