#!/usr/bin/env python3
"""decode_v9_k0 -- v8b framing + PRED-WRAP value reconstruction (GT-free).

VALUE_SPLICE_HUNT finding: V-record payloads are LOW-side windows of the new
value; bytes above the window are NOT copied from the register -- they are
recovered by nearest-wrap around a prediction (parallelogram pred = 2*last -
last2 per axis). The old "carry +-1..4 on byte k0-1" search was a broken
approximation of this wrap (and accepted wrong-sign values whenever they were
in band). Placement (k0_rule) is unchanged; hi-byte fill + fallback replaced:

  r2_v9(R, payload, T, axis, pred):
    k0r = k0_rule(T, nb)                      (byte-role rule, unchanged)
    c1  = placement k0r, hi bytes = wrap-nearest-pred, tail bytes = R
    c2  = low-aligned placement (k0 = 8-nb), hi bytes = wrap-nearest-pred
    first in-band of (c1, c2) wins; else old closest-to-prev scan.

No GT anywhere in the decode; DXF used for SCORING ONLY.
Usage: python decode_v9_k0.py <case>
"""
import struct, sys
import numpy as np
...
def hi_cands(R, payload, k0, a, ref_v, span=4):
    """placement k0, tail from R; hi bytes = R's hi +- dh. Return the IN-BAND
    candidate NEAREST ref_v (the previous value). dh=0 wins ties."""
    nb = len(payload); hb = k0
    if hb < 0 or k0 + nb > 8: return None
    tail = R[k0 + nb:]
    ...
    for dh in range(-span, span + 1):
        ...
        if best is None or dv < best[0]: best = (dv, vb)
    return best[1] if best else None
def r2_v9(R, payload, T, a, pred_v):
    nb = len(payload)
    if nb == 0 or nb > 8: return None
    k0r = k0_rule(T, nb)
    if k0r + nb > 8: k0r = 8 - nb
    last = be(R)
    # rule placement: candidates hi(R)+-dh, choose in-band nearest LAST
    c1 = hi_cands(R, payload, k0r, a, last)
    if c1 is not None: return c1
    # rule placement found nothing in band: low-aligned alternate
    k0l = 8 - nb
    if k0l != k0r:
        c2 = hi_cands(R, payload, k0l, a, last)
