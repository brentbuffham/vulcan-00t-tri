...
def k0_rule(T,nb):
    # FLIPPED rule from mech_labels census: T1==0x20 -> k0=2; 0x21..0x5F -> k0=3
    if T is not None:
        T1,T2=T
        if T1==0x20: return 2
        if 0x21<=T1<=0x5F: return 3
...
tree=cKDTree(Gu)
dist,_=tree.query(P)
print(f'v3e GT-FREE decode: {len(P)} vertices (GT {len(Gu)})')
...
with open(sp+r'\det_v3e.xyz','w') as f:
    for x,y,z in P: f.write(f'{x:.4f},{y:.4f},{z:.4f}\n')
print('wrote det_v3e.xyz')
