#!/usr/bin/env python3
"""REBUILD rails.pkl + P_base.npy from raw .00t (both died with old scratchpads).
Machinery lifted verbatim from topo_strip_decoder.py (opus 2026-07-07) but GT/PBASE
point at the locally-rebuilt files, and it DUMPS the two artifacts the alloc*.py
scripts expect:
  - rails.pkl  = (groups, rails, ref_events)
  - P_base.npy = baseline v5-P2 coord decode (guide off)
Prints the reproduction checks so we can confirm the rebuild is faithful to the
resume-doc numbers (topology groups, rails, anchor stats) before trusting it.
"""
...
GTCSV = 'intercepts_gt.csv'          # locally rebuilt
...
print('geo_end', geo_end, 'face_start', face_start)
...
print('saved rails.pkl')

# ================= v5-P2 coord decode (guide off) -> P_base.npy =========
...
P = run()
np.save('P_base.npy', P)
print('saved P_base.npy  shape', P.shape)

# ---------------- reproduction check: anchor + XZ-exact map size ----------------
gt = np.loadtxt(GTCSV, delimiter=','); Gu = np.unique(gt, axis=0); kd = cKDTree(Gu)
dist, _ = kd.query(P); anchor = dist < 0.002
print(f'anchor slots (<2mm 3D): {anchor.sum()} / {len(P)}  ({anchor.mean()*100:.1f}%)')

kd2 = cKDTree(gt[:, [0, 2]]); cand = kd2.query_ball_point(P[:, [0, 2]], 0.002)
gtcount = Counter(); s2g = {}
for s, cs in enumerate(cand):
    if len(cs) == 1: s2g[s] = cs[0]; gtcount[cs[0]] += 1
s2g = {s: g for s, g in s2g.items() if gtcount[g] == 1}
print(f'XZ-exact unique-both-ways matches: {len(s2g)}   (resume says 970; 3D-exact was 693)')
