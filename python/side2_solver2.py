...
assign_rid = {}
def score_cand(rid, sgn, ci):
    """higher = better: cross-family occupancy CONFIRMS (the two triangle
    families of one gap share positions); same-family occupancy conflicts."""
    S = centers[ci]
    pos = positions(rid, S)
    same = sum(min(occ[(ci, sgn)][p], 2) for p in pos)
    cross = sum(min(occ[(ci, -sgn)][p], 2) for p in pos)
    return cross - same
...
for _, _, rid in order:
    ...
    for sgn, (ci, pk) in cc.items():
        sc = score_cand(rid, sgn, ci)
        key = (sc, strength(pk)[0])
        if best is None or key > best[0]:
            best = (key, sgn, ci)
    _, sgn, ci = best
